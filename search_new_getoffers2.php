...
if( $_POST ){
	...
	$setcat = " or posted_offers.offer_cat like '".$cat."'   ";
	...
	
	{
		...
		$srch .=" posted_offers.offer_title like '%".$ext[$i]."%'";
		
		else
		
		$srch .=" or posted_offers.offer_title like '%".$ext[$i]."%'";
	...
	$srch .=" or posted_offers.offer_price = '$".$exp1[0]."'";
  ...
  $arrayget = $obj->selectAll2('posted_offers RIGHT JOIN bids_against_offers ON posted_offers.offer_id = bids_against_offers.offer_id','posted_offers.*, count(bids_against_offers.offer_id)'," ".$srch."  ".$setcat."  ",' GROUP BY posted_offers.offer_id order by posted_offers.offer_id DESC ');
  ...
  $query = mysqli_query($conn,"select posted_offers.*, count(bids_against_offers.offer_id) from posted_offers RIGHT JOIN bids_against_offers ON posted_offers.offer_id = bids_against_offers.offer_id where ".$srch."  ".$setcat." GROUP BY posted_offers.offer_id order by posted_offers.offer_id DESC");

           
  $countr = mysqli_num_rows($query);
