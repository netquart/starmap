...
if( $_POST ){
	...
	$setcat = " or offer_cat like '".$cat."'   ";
	 ...
	 $message = strip_tags(trim($_POST['txt_fname']));
    
	$hashtag = gethashtags($message);
	
	if($hashtag!='')

    $setcat = " or hashtags like '".$hashtag."'   ";
 ...
 foreach ($arrayget as $array)
 ...
 
  {
	 ...
	 $checkoffer = $obj->madeoffercheck($array['offer_id']);
	 
	 if($checkoffer)
	 
	 $offer_button = '<button style="font-family:Arial, Helvetica, sans-serif; font-size:14px;" type="button" class="btn btn-sm btn-primary buttonred"  onclick="moreinfo('.$array['offer_posted_user'].','.$array['offer_id'].','.$_SESSION['user_session'].',\''.$tags.'\',\'make_offer_'.$array['offer_id'].'\');">more info</button>';
	 
	 else
	 
	 $offer_button = '<button style="font-family:Arial, Helvetica, sans-serif; font-size:14px;" type="button" class="btn btn-sm btn-primary buttonred"  onclick="makeoffer('.$array['offer_posted_user'].','.$array['offer_id'].','.$_SESSION['user_session'].',\''.$tags.'\',\'make_offer_'.$array['offer_id'].'\');">make an offer</button>';
	 
	 
	 
	 
	 
	$stt .= $tags.'</div><div style="font-family: Arial,Helvetica,sans-serif; font-size: 13px;  padding-left: 0px;" class="col-lg-3 col-xs-3 col-sm-3 col-md-3">'.$obj->bidscount($array['offer_id']).$checkoffer.'</div><div  class="col-lg-4 col-xs-4 col-sm-4 col-md-4" style="padding-right: 0px; font-weight: bold; color: red; text-align: right;padding-left:0px;">'.$offer_button.'</div></div></div>';
  ...
  echo $stt.'#@#'.$countr;	
	  ...
	  echo '<div style="float:none;color:#333; padding:25px 10px 10px 5px;text-align:center;clear:both;">no record found!</div>';
